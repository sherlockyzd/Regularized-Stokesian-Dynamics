...
! QQ  5NNx5NN
      ...
      SUBROUTINE PER_GRPERY_MOB(APP,APQ,AQQ,CONF,RADII,NN,LATTICE,EWS_ERR)
      IMPLICIT NONE
      INTEGER,intent(in):: NN
      real(8),intent(out)::APP(6*NN,6*NN),APQ(6*NN,5*NN),AQQ(5*NN,5*NN)
      REAL*8,intent(in):: CONF(3,NN),RADII(NN),LATTICE(3,3),EWS_ERR
...
!$omp parallel default(NONE)shared(a,c,m,NN,Radii)&
...
!$omp do schedule(static)
  self: DO alpha=1,NN
    ...
    !write(*,*) 'thread_id=',thread_id
        beta=alpha
       CALL PER_ROTNE_PRAGER_SELF(TTS,RRS,QQS_TR,RADII(alpha))
!!$omp critical
!!$omp flush(a,c,m)
        a(alpha,beta,:,:)=TTS
        c(alpha,beta,:,:)=RRS
...
!!!!!!$omp reduction( +:)
!$omp parallel default(NONE)shared(a,b,c,g,h,m,NN,RADII,p_pos,correction_method)&
...
pairwisealpha: do alpha=1,NN
     pairwisebeta: do beta=alpha+1,NN
        ...
        CALL PER_ROTNE_PRAGER_IJ(TTP,RRP,TRP,QQP_TR,GPQ_TR,HPQ_TR,R,aaI,aaJ)
