module Prtcl_DEMSystem
  ...
  contains
    ...
    procedure:: Finalize => DEMS_Finalize
...
!   predefined plane 
!********************************************************************************
  subroutine  DEMS_Initialize(this,chDEMPrm,CONF,RADII,L_mag)
    implicit none
    class(DEMSystem)::this
    character(*),intent(in)::chDEMPrm
    real(RK),intent(in)::L_mag
    real(RK),intent(in),dimension(:)::RADII
    ...
    call DEMProperty%InitPrtclProperty(chDEMPrm,RADII(1))
...
#else
    if(.not.DEM_Opt%RestartFlag) then
      call GPrtcl_list%MakingAllPrtcl(chDEMPrm,CONF,L_mag)
      if(nrank==0) then
...
!********************************************************************************
!   Finalizing DEMSystem 
!********************************************************************************
  subroutine DEMS_Finalize(this)
    implicit none
    class(DEMSystem)::this
    
    call DEMProperty%Finalize()
    call DEMGeometry%Finalize()
    call GPrtcl_list%Finalize()
    call GPPW_CntctList%Finalize() 
    call DEMContactSearch%Finalize()
    call DEMContactSearchPW%Finalize()
  end subroutine DEMS_Finalize
