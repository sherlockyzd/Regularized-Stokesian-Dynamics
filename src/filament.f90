...
!****************************************************************
  ...
  use SYS_property,only:mass_par,DT_DEM
  USE LATTICE_BASE,only:LB
  use filament,only:F_rb,Filament_num,Filament_tau_base,Filament_Inertial_body_inverse,index1
 ...
 !,new_U_par_filament,new_conf_filament,new_q_filament
  ...
  subroutine BDF_solver(Nfilament,iter,error,Yk_next,X1_now,X1_past,U1_now,tau_now,Lie_algebra_now, &
                    & Inertial_torque,Fe,conf_now,conf_past,U_now,q)
  IMPLICIT NONE
  integer,intent(in):: Nfilament,iter
  real*8,intent(in)::X1_now(3*Nfilament),X1_past(3*Nfilament),U1_now(3*Nfilament),Lie_algebra_now(3*Nfilament)
  ...
  integer::ii
  real*8:: conf_next(3,Nfilament),omega_next(3,Nfilament),dYk_norm,fy_norm
  real*8:: Lie_algebra_next(3*Nfilament),X1_next(3*Nfilament),Inertial_lamda(3*Nfilament),tau_next(3,Nfilament)
  real*8:: dYk(6*Nfilament),filament_Fy(6*Nfilament),Ck(6*Nfilament),Dk(6*Nfilament)
  ...
  do ii=1,iter
    ...
    call Lie_algebra_To_q(Nfilament,q,Lie_algebra_next,q_next)
    ...
    call filament_function(Nfilament,Yk_next,X1_now,X1_past,U1_now,tau_now,Lie_algebra_now, &
    ...
    dk_mat=Jacobian_filament+CDk_sum
    call MATREV(dk_mat,6*Nfilament)
  ...
  function matrix_mul77(a,b,bc)  result(c)                       ! Fortran77矩阵相乘
  implicit none
  ...
  do ii=1,F_rb
    ...
    do jj=1,Nfilament-F_rb
      if(jj.eq.index1_internal) then
  ...
  subroutine filament_function(Nfilament,Yk_next,X1_now,X1_past,U1_now,tau_now,Lie_algebra_now, &
                    & Inertial_torque,Fe,conf_now,conf_past,conf_next,U_now,omega_next,filament_Fy)
  IMPLICIT NONE
  integer:: Nfilament
  real*8,intent(in)::Yk_next(6*Nfilament)
  ...
  !type(quaternion):: q_next(Nfilament)
  real*8 Fe_ext(3*Nfilament),Te_ext(3*Nfilament),dexpu(3)
  real*8 X1Inertial_lamda(3*F_rb),X1_next(3*F_rb),Inertial_lamda(3*Nfilament),Lie_algebra_next(3*Nfilament)
  ...
  do  KK_fila=1,F_rb       
      ...
      num_fila_sum=num_fila_sum+num_fila
  enddo
  write(*,*) 'index_Y2==',index_Y2



  do ii=1,Nfilament
      dexpu=dexpu_inv(Lie_algebra_next(3*(ii-1)+1:3*ii),omega_next(3*(ii-1)+1:3*ii))
      filament_Fy(3*Nfilament+3*(ii-1)+3*Nfilament+1:3*ii)=Lie_algebra_next(3*(ii-1)+1:3*ii) &
